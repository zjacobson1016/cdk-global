"""Flask conversation UI for the Parts Invoice Processing Agent.
...
FORMAT_SYSTEM_PROMPT = (
    "You are a formatting assistant. The user asked a question to an invoice "
    "processing agent and received a raw response. Rewrite the raw response into "
...
def _to_str(val) -> str:
    """Ensure a value is a plain string, serializing dicts/lists to JSON."""
...
def _extract_text(data: dict) -> str:
    """Walk common agent response shapes and return the assistant text."""
    # ResponsesAgent: {"output": [{"role": "assistant", "content": ...}]}
    if "output" in data:
        output = data["output"]
...
def _format_with_llm(user_question: str, raw_response: str) -> str:
    """Send the raw agent output through an LLM to produce pretty markdown."""
...
def _call_agent(messages: list[dict]) -> str:
    """Send messages to the agent serving endpoint, then format the response."""
    headers = cfg.authenticate()
    headers["Content-Type"] = "application/json"
...
def debug():
    """Return the raw agent response for troubleshooting."""
    body = request.get_json()
